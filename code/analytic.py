...
def MakeBabyBoom():
    ...
    thinkplot.Config(
        xlabel='minutes', ylabel='CCDF', yscale='log', legend=False)
...
def MakeNormalCdf():
    ...
    for mu, sigma in zip(mus, sigmas):
        xs, ps = thinkstats2.RenderNormalCdf(
            mu=mu, sigma=sigma, low=-1.0, high=4.0)
        label = r'$\mu=%g$, $\sigma=%g$' % (mu, sigma)
        thinkplot.Plot(xs, ps, label=label)

    thinkplot.Save(
        root='analytic_normal_cdf',
        title='Normal CDF',
        xlabel='x',
        ylabel='CDF',
        loc=2)
